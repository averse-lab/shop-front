...
const MaintenancePage: FC<IProps> = async (props) => {
  const { lang } = props.params;
  const dictionary = await getDictionary(lang);

  const { maintenance } = dictionary;
  ...
  return (
    ...
    >
      <div className={clsx("h-10 w-10 md:h-16 md:w-16")}>
        <svg
      ...
      <p className={clsx("text-sm md:text-lg", "text-center", "text-black")}>
        {maintenance.message}
      </p>
    </div>
  );
